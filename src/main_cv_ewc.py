...
MODEL_LOAD_FOLDER = os.path.join(config.model_load_folder, ROI)
MODEL_SAVE_FOLDER = os.path.join(config.model_save_folder, ROI)
...
for cv_num in range(4):
    ...
    # ------------------------ Test the model
        ...
        model_load_folder = os.path.join(MODEL_LOAD_FOLDER, str(cv_num))
        model_save_folder = os.path.dirname(MODEL_SAVE_FOLDER)
        model_save_folder = os.path.join(model_save_folder, 'ukb', v)
        results_folder = os.path.join(RESULTS_FOLDER + '_tf_ewc', str(cv_num))
        print("Model Load Path:         ", model_load_folder)
        print("Model Save Path:         ", model_save_folder)
        print("Results Path:         ", results_folder)

        # test_dataset related
        valid_dataset_old = Region_Dataset(config.old_root, dataset_df_o, valid_indices_old[cv_num], ROI)
        dataloader_valid_old = DataLoader(valid_dataset_old, 
                                    batch_size=BATCH_SIZE, 
                                    sampler=RandomSampler(valid_dataset_old),
                                    collate_fn=valid_dataset_old.collate_fn,
                                    pin_memory=True,
                                    num_workers=N_WORKERS)
        ...
        trainer = CNN_Trainer(model=model, 
                            ...
                            dataloader_test=dataloader_valid_old,
                            dataloader_train_old=None, 
                            epochs=None, 
                            optimizer=None, 
                            early_stopping=None,
                            scheduler=None,
